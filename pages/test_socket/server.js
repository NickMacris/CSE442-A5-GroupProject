...
app.get("/", (req, res) => res.sendFile(__dirname + "/index.html"));
 ...
 io.on("connection", function(socket) {
  io.emit("user connected",JSON.stringify("Hello via Json"));
  clients += 1;
  console.log('User #' + clients+' has been added');

  //send chat history
  if(chat_history.length > 0){
      io.emit('chat_history', chat_history);
  }
 ...
 */
 function vote(){
  ...
  if(movie_list.length > movie_cntr){
      ...
      io.emit('movie', movie_list[movie_cntr].get('movie_data'));
      console.log("Sent: "+ movie_list[movie_cntr].get('movie_data'));
      movie_cntr += 1;
  }
  else{
      end_vote();
...
function end_vote(){
  voted = 0;
  movie_cntr = 0;
  io.emit('vote_result',favorite_movie);
